function CT_CertaintyScale(subject, run_number)
%% Isabel 25062017, Kristin 
% 

% CT_CertaintyScale('test', 1)
...
SETTINGS.setup = 'UMG_Psychphysic';  % OfficePC_Isabel,'OfficePC_Kristin ''
SETTINGS.doECG = 1; % EXPERIMENT WITH ECG RECORDING (BINARY)
Nr_Repitions = 1;  % 5 time periods -> 10 trials
NrTrials_Pre = [3 6 ]; %NrTrials_Pre = [4 6 12 15 18]; 
...
switch SETTINGS.setup
    case 'OfficePC_Isabel'
        ...
        path_save                           = 'C:\Users\IKreis\Documents\MATLAB\Data';
...
while true
    filename = [subject '_' datestr(date,'yyyy-mm-dd') '_' 'CT' '_' num2str(fileNumber,'%02u')];
...
%% TRIGGER CODES
%for resting_baseline (fixation red circle), start_counting(beep and green circle), end_counting, report_count, wagering, ITI_fix
%trigger.start_count = 3;
%trigger.end_count = 4;
%trigger.report = 5;
%trigger.wager = 6;
%trigger.ITI_fix=7;
...
% SETTINGS.gray_target_color               = [150 150 150];
% SETTINGS.red_fix_bright                  = [250 0 0];
% SETTINGS.yellow_color                    = [255 255 0];
% SETTINGS.gray_wager_color                = [100 100 100];
% SETTINGS.gray_rectangular                = {SETTINGS.gray_wager_color, SETTINGS.gray_wager_color/3};
...
count_durationsPreCertainty         = Shuffle(repmat([24,34,44,54,64],1,1));%initiates the vector of shuffled different presentation lengths in secs (one duration for each trial - excl. the instruction trial 1). will be updated in the endd of each trial.
min_duration_instr = 2; %seconds
max_duration_instr = 120; %seconds
bs_duration= 180; %180 / 3 minutes baseline
%specific counting_interval defined below per trial
stop_duration=1; %seconds
ITI_duration=20; %seconds
...
%%%black background screen is opened
...
while Trial <= (SETTINGS.ntrials ) % Main Loop starts
    ...
    if Trial==1 %trial 1 is used for instruction + the baseline period!
        ...
        DrawFormattedText(SETTINGS.window,'Willkommen!\n\nDie Aufgabe beginnt mit einem kurzen Ruheintervall, in dem Sie \n\nlediglich entspannt auf den Bildschirm schauen sollen.\n\nDanach folgt die Aufgabe zur Zeitschätzung. Sie sollen dann jeweils \n\nschätzen, wieviele Sekunden zwischen dem Erscheinen des grünen\n\nund des roten Kreises vergangen sind.\n\nDanach sollen Sie angeben, wie sicher Sie sich sind, dass Ihre \n\nSchätzung korrekt war.\n\nBitte drücken Sie ENTER um zu beginnen.','centerblock','center',SETTINGS.white_color);
        Screen(SETTINGS.window,'Flip');
    ...
    elseif Trial>1
        ...
        if Trial == NrTrials_Pre(1) ||  Trial == NrTrials_Pre(2) 
            Screen('TextSize',SETTINGS.window,SETTINGS.size_text); %sets textsize for instructions
            DrawFormattedText(SETTINGS.window,'Bitte gib an wie du deine allgemeine Leistung in dieser Aufgabe einschätzt.  Bist du oft...  \n\n Zum Starten einmal die Pfeiltaste nach UNTEN drücken.',500,500,SETTINGS.white_color);
            Screen(SETTINGS.window,'Flip');
        ...
        Screen('TextSize',SETTINGS.window,SETTINGS.size_text); %sets textsize for instructions
        DrawFormattedText(SETTINGS.window,'Wie viele Sekunden sind vergangen?\n\n Geben Sie ihre Schätzung mit dem Nummernblock ein und bestätigen Sie mit ENTER..',50,70,SETTINGS.white_color);
        Screen(SETTINGS.window,'Flip');
        ...
        while ( enterpressed==0 )
            ...
            if (pressed && ~enterpressed) %keeps track of key-presses and draws text
                ...
                Screen('TextSize',SETTINGS.window,SETTINGS.size_text); %sets textsize for instructions
                DrawFormattedText(SETTINGS.window,'Wie viele Sekunden sind vergangen?\n\nGeben Sie ihre Schätzung mit dem Nummernblock ein und bestätigen Sie mit ENTER.',100,70,SETTINGS.white_color);
        ...
        if SETTINGS.wager
            ...
            Screen('TextSize',SETTINGS.window,SETTINGS.size_text); %sets textsize for instructions
            DrawFormattedText(SETTINGS.window,'Wie sicher sind Sie, dass die soeben angegebene Anzahl an Sekunden korrekt war.  \n\n Zum Starten einmal die Pfeiltaste nach UNTEN drücken.',500,500,SETTINGS.white_color);
            Screen(SETTINGS.window,'Flip');
    ...
    %% END
    %do sth similar for trigger logfile here: [save_Eye_Time,save_Eye_x ,save_Eye_y, index_saveEye] = save_eye_Data(index_saveEye,save_Eye_Time,save_Eye_y,save_Eye_x, eye_offset_x, eye_offset_y);
    ...
    %timing variables
    trial(Trial).RunStartTime            = RunStartTime;
    ...
    trial(Trial).position_wager_chosen_post                = position_wager_chosen_post; %picked wager-position  in the rectangular frame (1 is the top wager, 6 the bottom wager)
